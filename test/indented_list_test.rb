...
class IndentedListTest < Minitest::Test
  ...
  def test_method_missing
    assert_equal @list.count, @list.list.count
  end
  
  def test_iterate_list
    expected = ["887 - NON-MARC INFORMATION FIELD (R)",
                 {"Indicators"=>
                   {"First - Undefined"=>["# - Undefined"],
                    "Second - Undefined"=>["# - Undefined"]},
                  "Subfield Codes"=>
                   ["$a - Content of non-MARC field (NR)", "$2 - Source of data (NR)"]}]
    actual = ''
